...
// Forward declaration of BoundaryWall
class BoundaryWall;
...
// Implement Point::intersects_line after BoundaryWall is defined
bool Point::intersects_line(const BoundaryWall& line) const {
...
public:
    ...
    void draw(sf::RenderWindow& window) override {
        ...
        // Update position if the left mouse button is pressed
    ...
    void draw_rays(sf::RenderWindow& window) {
        ...
        // Real impact: FPS up from ~40 to ~90 with 46080 rays!
        window.draw(lines);
    }
    ...
    Point intersects_any_line(const Point& new_pos, Directions direction) {
        ...
        for (std::size_t i = 0; i < WALLS.size(); ++i) {
            if (temp_line.intersects_line(*WALLS[i])) {
                ...
                if (std::fabs(wall_angle - PI) < EPSILON || std::fabs(wall_angle + PI) < EPSILON ||
                    std::fabs(wall_angle - HALF_PI) < EPSILON || std::fabs(wall_angle + HALF_PI) < EPSILON) {
                    return pos;
                }
